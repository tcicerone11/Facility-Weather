...
def fetch_taf(icao):
    ...
    return {
        ...
        "thunder": "TS" in upper,
        "snow": bool(
            re.search(r"\bSN\b|\bSHSN\b|\bBLSN\b", upper)
        ),
        "freezing_precip": bool(
            re.search(r"\bFZRA\b|\bFZDZ\b", upper)
        ),
    }
...
def nws_point_data(lat, lon):
    point = get_json(
        f"{NWS}/points/{lat:.4f},{lon:.4f}"
    )

    p = point.get("properties", {})

    hourly = (
        get_json(p["forecastHourly"])
        if p.get("forecastHourly")
    ...
    grid = (
        get_json(p["forecastGridData"])
        if p.get("forecastGridData")
        else {"properties": {}}
    )
...
def evaluate(facility, settings):
    ...
    point, hourly, forecast, grid = nws_point_data(
        lat,
        lon,
    )
    ...
    alerts = merge_alerts(
        fetch_point_alerts(lat, lon),
        fetch_zone_alerts(zone),
    )
    ...
    precip = value_series(
        gp.get("quantitativePrecipitation"),
        "precip",
        168,
    )

    snow = value_series(
        gp.get("snowfallAmount"),
        "snow",
        168,
    )
    ...
    thunder_cutoff = (
        now_utc()
        + timedelta(
            hours=settings["thunder_watch_hours"]
        )
    )
    ...
    for p in periods:
        ...
        text = (
            f"{p.get('shortForecast') or ''} "
            f"{p.get('detailedForecast') or ''}"
        ).lower()

        if (
            dt
            and dt <= thunder_cutoff
            and "thunder" in text
        ):
            thunder_soon = True
            break
        ...
        if event in CLOSE_EVENTS:
            add(
                ...
                (
                    ...
                    "Follow the facility's emergency "
                    "or closure procedure. "
                    f"Source: {source}."
                ),
            )
            ...
            add(
                ...
                a["headline"] or event,
                (
                    "Review and implement the protective "
                    "action in the facility plan. "
                    f"Source: {source}."
                ),
            )
            ...
            add(
                ...
                a["headline"] or event,
                (
                    "Monitor updates and prepare staff "
                    f"or equipment. Source: {source}."
                ),
            )
    ...
    if peak_gust is not None:
        ...
        if peak_gust >= settings["wind_close_kt"]:
            add(
                "close",
                "Strong wind threshold reached",
                (
                    "Observed or forecast gusts may reach "
                    f"about {mph:.0f} mph "
                    f"({peak_gust:.0f} kt)."
                ),
                (
                    "Follow your approved high-wind "
                    "closure or restriction procedure."
                ),
            )
            ...
            add(
                ...
                (
                    "Secure vulnerable outdoor items "
                    "and take the protective action "
                    "defined by the facility plan."
                ),
            )
            ...
            add(
                ...
                (
                    "Monitor wind conditions and prepare "
                    "for outdoor restrictions if winds increase."
                ),
            )
    ...
    if snow_7d >= settings["snow_close_in"]:
        add(
            "close",
            "Heavy snow threshold reached",
            (
                "The 7 day outlook contains about "
                f"{snow_7d:.1f} inches of forecast snow."
            ),
            (
                "Follow the facility's approved severe "
                "snow or closure procedure."
            ),
        )
        ...
        add(
            ...
            (
                "Prepare snow response resources and "
                "review staffing or access plans."
            ),
        )
        ...
        add(
            "watch",
            "Snow may affect operations",
            (
                "The 7 day outlook contains about "
                f"{snow_7d:.1f} inches of forecast snow."
            ),
            (
                "Monitor forecast changes and begin "
                "snow response planning."
            ),
        )
    ...
    if precip_7d >= settings["precip_action_in"]:
        add(
            ...
            (
                f"About {precip_7d:.1f} inches of "
                "liquid precipitation is represented "
                "in the 7 day outlook."
            ),
            (
                "Review drainage, flooding, outdoor work, "
                "and access concerns."
            ),
        )
        ...
        add(
            "watch",
            "Wet weather may affect operations",
            (
                f"About {precip_7d:.1f} inches of "
                "liquid precipitation is represented "
                "in the 7 day outlook."
    ...
    if thunder_soon:
        add(
            "watch",
            "Thunderstorms may affect the facility",
            (
                "A thunderstorm signal appears in the next "
                f"{settings['thunder_watch_hours']} hours "
                "in the local or airport forecast."
            ),
            (
                "Monitor official warnings and your approved "
                "lightning procedure. This system does not "
                "measure lightning strike distance."
            ),
        )
...
.legend-item.watch{
    border-top:5px solid var(--watch);
}
...
.legend-item.close{
    border-top:5px solid var(--close);
}
...
<div class="intro">
Weather translated into operational awareness
for facility managers.
</div>

<div class="test-notice">
<strong>TEST SYSTEM.</strong>
The thresholds on this dashboard are examples
for development and testing.
They are not approved operating,
restriction, or closure policy.
</div>
...
Normal operations.
No configured weather threshold currently
suggests an operational change.
</div>

<div class="legend-item watch">
<strong>WATCH</strong>
Weather could affect operations.
Monitor conditions and prepare staff
or equipment.
</div>

<div class="legend-item action">
<strong>ACTION</strong>
A significant condition has been reached
or is expected.
Take the protective action in the facility plan.
</div>

<div class="legend-item close">
<strong>CLOSE CRITERIA MET</strong>
A configured closure threshold
or critical warning has been reached.
Follow approved closure
...
<div class="footer">
This system uses exact facility coordinates
for NWS point forecasts and point-based alerts,
plus the configured broader NWS zone
for additional context.
Airport facilities can also use airport
observations and airport short-term forecasts.
This dashboard does not provide
live lightning-strike distance.
</div>
...
function card(r){
    ...
    const met=r.metar||{};
    const taf=r.taf||{};
    const sc=r.status_copy||{};

    const reasons=(r.reasons||[]).length
        ? r.reasons.map(x=>`
            <div class="reason ${e(x.level)}">
                <b>${e(x.title)}</b><br>
                ${e(x.detail)}
                <span class="do">
                    What to do:
                    ${e(
                        x.action
                        || 'Review the facility operating plan.'
                    )}
                </span>
            </div>
        `).join('')
        : `
            <p class="muted">
                No configured operational threshold
    ...
    const alerts=(r.alerts||[]).length
        ...
        : `
            <p class="muted">
                No active official NWS alerts affecting
                the exact facility point or configured
                broader zone.
            </p>
        `;

    const forecast=(r.forecast||[]).map(p=>`
        <div>
            <b>${e(p.name)}</b><br>
            ${e(p.temperature)}°${e(p.temperatureUnit)}<br>
            ${e(p.shortForecast)}<br>
    ...
    const airportCurrent=r.icao
        ? `
            <h3>Current Airport Conditions</h3>
            ...
            <div class="grid">
                ...
                <div class="metric">
                    ...
                    <strong>
                        ${
                            met.temperature_f==null
                            ? '—'
                            : e(met.temperature_f)+'°F'
                        }
                    </strong>
                </div>
                ...
                <div class="metric">
                    ...
                    <strong>
                        ${
                            met.visibility_sm==null
                            ? '—'
                            : e(met.visibility_sm)+' mi'
                        }
                    </strong>
                </div>
    ...
    const technical=r.icao
        ? `
            <details>
                <summary>
                    Technical airport weather details
                </summary>

                <p class="small">
                    METAR is the coded current airport
                    observation.
                    TAF is the coded airport
                    short-term forecast.
                </p>

                <b>
                    Raw current airport observation,
                    ${e(r.icao)} METAR
                </b>

                <code>
                    ${e(met.raw||'Unavailable')}
                </code>

                <br>

                <b>
                    Raw airport short-term forecast,
                    ${e(r.icao)} TAF
                </b>

                <code>
                    ${e(taf.raw||'Unavailable')}
                </code>
    ...
    const meta=[
        r.icao
            ? `Airport station ${e(r.icao)}`
            : '',
        r.zone
            ? `NWS zone ${e(r.zone)}`
            : '',
        `${e(r.latitude)}, ${e(r.longitude)}`
    ]
    .filter(Boolean)
    .join(' · ');
    ...
    return `
        <section class="facility">
            ...
            <div class="body">
                ...
                <p>
                    <strong>
                        ${
                            e(
                                sc.action
                                || 'Review current conditions and facility policy.'
                            )
                        }
                    </strong>
                </p>
                ...
                <h3>
                    Why is the facility at this level?
                </h3>
